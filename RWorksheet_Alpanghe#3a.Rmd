...
```{r}
x <- matrix(c(1:8, 11:14), nrow=3, ncol= 4, byrow=TRUE )
x
```
...
```{r}
rownames(times2) <- c("isa", "dalawa", "tatlo")
colnames(times2) <- c("uno", "dos", "tres", "quatro")
times2
```
...
```{r}
reshaped_mat <- matrix(x, nrow=6, ncol=2)
reshaped_mat
```
...
array_3d <- array(repeated, dim = c(2, 4, 3))
array_3d
```
...
dimnames(array_3d) <- list(rownames(array_3d), colnames(array_3d), c("1st-Dimensional Array", "2nd-Dimensional Array", "3rd-Dimensional Array"))
array_3d
```
